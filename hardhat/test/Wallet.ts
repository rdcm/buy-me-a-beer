...
describe("Wallet", function () {
  ...
  describe("Happy paths", function () {
    it("Should return increased balance after top up", async function () {
      ...
      const { wallet, other1 } = await loadFixture(deployWaletFixture);
    ...
    it("Should have 0 ether after success withdraw", async function () {
      const amount = hre.ethers.parseEther("321.0");
      const { wallet, other1, other2 } = await loadFixture(deployWaletFixture);
      ...
      expect(await hre.ethers.provider.getBalance(other2)).to.eq(
        amount + targetInitBalance,
      );
    });
  });
  ...
  describe("Negative cases", function () {
    it("Should revert with the right error if called from another account", async function () {
      ...
      await expect(wallet.connect(other1).withdraw(other2)).to.be.revertedWith(
        "fuck off",
      );
    });
    ...
    it("Should not allow pass 0 address for withdraw", async function () {
      ...
      await expect(wallet.withdraw(hre.ethers.ZeroAddress)).to.be.revertedWith(
        "fuck off again",
      );
    });
  });
